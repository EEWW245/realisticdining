...
public class DrinkAnimRegistry {
     ...
     * @param holdDuration      v2.1.4+ 持物前缀时长（秒）；> 0 启用，0 走旧 IDLE↔DRINK 路径
     * @param hideLeftArm       v2.1.4+ 持物阶段是否隐藏 Left Arm 骨骼
     *
    ...
    /**
     * v2.1.3+ 为已注册的饮料 handler 配置 pickup/putdown 动画。
     *
     * <p>附属在 {@code register} 之后调用本方法，让物品进入主手时自动播放"拿起"动画，
     * 播完后定格在最后一帧进入持物等待（HOLD 态），按 U 键播放 drink 动画，
     * 切出主手时播放"放下"动画。
     *
     * <p><b>动画文件要求</b>：pickup/putdown 动画必须与 drink 动画在同一个
     * animation.json 文件内（GeckoLib GeoModel 限制），且 geo 模型共用同一份。
     *
     * <p><b>未配置时</b>：handler 走旧路径（IDLE ↔ DRINK），不播放 pickup/putdown，
     * 保持向后兼容。
     *
     * @param handler             {@link #register} 返回的 handler
     * @param pickupRawAnimName   pickup 动画名（animation.json 内，如 "animation.bottle.pickup"）
     * @param pickupDuration      pickup 时长（秒，应与 animation.json 内的动画长度一致）
     * @param putdownRawAnimName  putdown 动画名（可为 null 表示不播放下动画，切物品时瞬间消失）
     * @param putdownDuration     putdown 时长（秒，putdownRawAnimName 为 null 时忽略，可传 0）
     */
