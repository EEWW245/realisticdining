...
/**
 * 材质包扩展物品的共用 GeoItem（Fabric 1.21.1）。
 *
 * <p>所有材质包扩展物品共用一个 {@code pack_empty} GeoItem（不注册到创造栏，
 * 玩家永远拿不到）。渲染时通过 {@link PackCustomRenderer} 替换为各扩展物品的
 * 模型/材质/动画（路径由定义文件名派生）。
 *
 * <p>动画控制器只检测一个 {@code eat} 控制器，挂载 {@code eat} + {@code pickup} 两个触发动画：
 * <ul>
 *   <li>按下 U 键 → {@link com.realisticdining.fabric.client.pack.PackKeyRouter}
 *       本地调用 {@link #triggerEatAnimation} 触发 {@code triggerAnim("eat", "eat")}</li>
 *   <li>主手切到 PICKUP 模式扩展物品 → {@link PackHandChangeTracker} 调
 *       {@link #triggerPickupAnimation} 客户端本地触发 {@code triggerAnim("eat", "pickup")}，
 *       播完后由 GeckoLib {@code hold_on_last_frame} 定格在持物姿态</li>
 *   <li>动画结尾的 {@code timeline: "finished;"} 自定义指令 → {@link #registerControllers}
 *       里的 {@code setCustomInstructionKeyframeHandler} 接收，执行：消耗物品 +
 *       {@code controller.stop()} + 解锁快捷栏；PICKUP 模式下若主手仍有物品则重新
 *       触发 pickup 定格（视觉无缝衔接）</li>
 *   <li>{@code sound_effects} 关键帧 → {@code setSoundKeyframeHandler} 接收，按定义文件
 *       的 sounds 映射播放音效</li>
 * </ul>
 *
 * <p>谓词根据 {@link #currentRenderItemId} 查 {@link PackDefinitionManager#getMode}：
 * PICKUP 返回 {@link PlayState#CONTINUE} 让 pickup 动画播放/定格，
 * STATIC 返回 {@link PlayState#STOP} 让 {@link PackHeldItemMotion} 接管程序化晃动。
 */
