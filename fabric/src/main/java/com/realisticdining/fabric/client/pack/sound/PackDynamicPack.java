package com.realisticdining.fabric.client.pack.sound;
...
public class PackDynamicPack implements PackResources {
    ...
    private static final PackMetadataSection METADATA =
            new PackMetadataSection(Component.literal("Provides dynamic sounds for pack extensions."), 34, Optional.empty());
    ...
    public void listResources(PackType type, String namespace, String prefix, ResourceOutput output) {
        ...
        if ("sounds.json".startsWith(prefix)) {
            output.accept(ResourceLocation.fromNamespaceAndPath(RealisticDining.MOD_ID, "sounds.json"),
                    () -> {
    ...
    public PackLocationInfo location() {
        return new PackLocationInfo("realisticdining_dynamic", METADATA.description(), PackSource.DEFAULT, Optional.empty());
    }
