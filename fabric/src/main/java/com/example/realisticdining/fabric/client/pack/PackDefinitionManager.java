...
public class PackDefinitionManager extends SimpleJsonResourceReloadListener
    ...
    protected void apply(Map<ResourceLocation, JsonElement> objectIn, ResourceManager resourceManager, ProfilerFiller profiler) {
        ...
        for (Map.Entry<ResourceLocation, JsonElement> entry : objectIn.entrySet()) {
            ...
            // Fabric 1.20.1：用 BuiltInRegistries.ITEM 查 Item 实例
            // 注意 BuiltInRegistries.ITEM.get 找不到时返回 Items.AIR（默认值），需用 getOptional 区分
            Item item = BuiltInRegistries.ITEM.getOptional(ResourceLocation.tryParse(itemId)).orElse(null);
            if (item != null) {
                itemList.add(item);
            }
    ...
    public ResourceLocation getFabricId() {
        return new ResourceLocation(RealisticDining.MOD_ID, "pack_definitions");
    }
