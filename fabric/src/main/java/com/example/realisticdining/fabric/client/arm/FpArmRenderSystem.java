...
import com.example.realisticdining.fabric.client.arm.drink.DrinkAnimHandler;
import com.example.realisticdining.fabric.client.arm.drink.DrinkAnimRegistry;
import com.example.realisticdining.fabric.client.arm.drink.DrinkAnimState;
...
public class FpArmRenderSystem {
    ...
    public static void triggerDrink(String drinkId) {
        ...
        ConsumeDrinkPacket.sendToServer(drinkId, true);
    }
    ...
    public static boolean isEatRiceActive(Player player) {
        ...
        boolean hasRiceInOffHand = com.example.realisticdining.compat.KaleidoscopeCompat.isCookedRice(offHandItem);
        boolean hasRiceInMainHand = com.example.realisticdining.compat.KaleidoscopeCompat.isCookedRice(mainHandItem);
    ...
    public static void updateDrinkState(PoseStack poseStack, MultiBufferSource bufferSource,
        ...
        // v2.2.1 孤儿 HOLD 防御（对齐 Forge 端行为 + 支持副手）：
        ...
        //   1) DRINK 动画播放中切物品被立即终止（Forge 端可自然播完）
        ...
        for (DrinkAnimHandler handler : DrinkAnimRegistry.all()) {
            ...
            // v2.x 提前消耗：DRINK 阶段提前量到达 → 提前发消耗包（抵消网络+服务端延迟）
            if (handler.consumeJustReady()) {
            ...
            if (oldDrinkId != null) {
                ...
                if (oldHandler != null) {
                    ...
                    if (oldPhase == DrinkAnimState.Phase.HOLD) {
                        ...
                        // （修复：旧逻辑在此误发 ConsumeDrinkPacket(false)，服务端会全物品栏
    ...
    /**
     * v2.1.8+ 读取瓶装饮料 NBT remaining_uses（1.20.1 用 ItemStack#getTag）。
     * 无 NBT 时返回 {@link Integer#MAX_VALUE} 作为"满（未消耗过）"哨兵值，
     * 这样比较时任何实际 remaining（1、2...）都小于它，无需知道 maxUses。
     */
    private static int getRemainingUses(ItemStack stack) {
        if (stack.isEmpty()) return 0;
        CompoundTag tag = stack.getTag();
        if (tag != null && tag.contains("remaining_uses")) {
