...
public class DrinkAnimModel extends GeoModel<GeoAnimatable> {
    ...
    public DrinkAnimModel(DrinkAnimConfig config, int group) {
        if (group == 2) {
            this.modelResource = new ResourceLocation(config.modId(), config.geoPath2());
            this.animationResource = new ResourceLocation(config.modId(), config.animationPath2());
        } else {
            this.modelResource = new ResourceLocation(config.modId(), config.geoPath());
            this.animationResource = new ResourceLocation(config.modId(), config.animationPath());
        }
    }
