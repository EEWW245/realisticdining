...
public final class BeerDrinkTracker {
    ...
    private static void applyDrunk(ServerPlayer player, int counter) {
        if (counter < 2) return;  // 1 瓶不会醉酒

        MobEffect effect;
        int duration;
        if (counter >= 6) {
            effect = ModEffects.WASTED.get();  duration = 90 * 20;  // 重度醉酒
        } else if (counter >= 4) {
            effect = ModEffects.DRUNK.get();   duration = 60 * 20;  // 中度醉酒
        } else {  // counter == 2 or 3
            effect = ModEffects.TIPSY.get();   duration = 30 * 20;  // 微醺
        }

        // remove 之前所有醉酒系列 effect，避免多个等级叠加显示
        player.removeEffect(ModEffects.TIPSY.get());
        player.removeEffect(ModEffects.DRUNK.get());
        player.removeEffect(ModEffects.WASTED.get());

        player.addEffect(new MobEffectInstance(
                effect,
                duration,
                0,  // amplifier 0：等级已通过 effect 类型本身区分
                false,  // ambient：非环境 effect，UI 显示完整进度条
                true,   // particles：显示粒子
                true    // icon：显示图标
        ));
    }

    /** 玩家下线时调用，清除累积计数（避免下次登录继承上次未完成的累积）。 */
