...
/**
 * 饮料/零食消耗服务端处理（1.21.1 版本，使用 DataComponents）。
 *
 * <p>客户端动画播完后发送 C2S 包，服务端在此处理：
 * <ul>
 *   <li>在玩家整个物品栏（主物品栏 + 副手）中查找匹配 drinkId 的物品</li>
 *   <li>瓶装饮料：读 CUSTOM_DATA.remaining_uses，递减；归 0 则 shrink(1)</li>
...
public final class DrinkConsumeHandler {
    ...
    public static void handle(ServerPlayer player, String drinkId) {
        ...
        // 1. 消耗物品（一次性 or 基于 CUSTOM_DATA 计数）
    ...
    /** 根据最大使用次数消耗物品：1 次直接 shrink；多次则基于 CUSTOM_DATA.remaining_uses 递减。 */
    private static void consumeItem(ItemStack stack, int maxUses) {
        ...
        int remaining = maxUses;
        CustomData existing = stack.get(DataComponents.CUSTOM_DATA);
        if (existing != null) {
            CompoundTag tag = existing.copyTag();
            if (tag.contains(REMAINING_USES_KEY)) {
                remaining = tag.getInt(REMAINING_USES_KEY);
            }
        }
